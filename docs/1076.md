...
*   [硒配蟒蛇](#Selenium-with-python)
*   [使用 Python 配置硒](#Configure-Selenium)
...
Selenium 是一个开源的测试工具，这意味着它可以从互联网上下载，不需要花任何钱。硒是一种功能测试工具，也与非功能测试工具兼容。

关于硒的更多信息，请参考以下链接:[https://www.javatpoint.com/selenium-tutorial](https://www.javatpoint.com/selenium-tutorial)

## 为什么我们使用硒进行自动化测试？

硒是最流行的自动化测试工具之一。在这里，自动化测试是使用自动化工具(如 **Selenium** )将任何手动测试用例转换成测试脚本的过程。

以下是一些方面，为什么我们更喜欢自动化测试的硒:
...
*   Python 支持面向对象编程方法来建立应用程序。它简单易学，提供了大量的高级数据结构。它是一种开源语言。
*   它是一种高级和解释器脚本编程语言。
...
并且，该命令将成功安装最新的**硒包**，即**硒-3.141.0** 添加到库中，如下图所示:

![Selenium with Python Tutorial](img/b55c40902ca2070d2e38198c4c15566b.png)

执行上述命令后，它将自动创建**硒文件夹**，其中包含所有硒库，如下图所示:
...
出于测试目的，我们将在 **Gmail** 应用程序上执行**登录测试**。

在本测试中，我们将自动化以下测试场景:

| 步伐 | 行动 | 投入 | 预期结果 |
| **1。** | 从硒导入网络驱动程序。 |  | 应该导入 Web 驱动程序。 |
| **2。** | 打开谷歌 Chrome 浏览器。 |  | 谷歌浏览器应该打开了。 |
...
一旦我们导航到 Gmail 应用程序的网址，我们将识别用户名文本框并传递它的值。
