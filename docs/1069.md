...
WebDriver提供了一些基本的浏览器导航命令，允许浏览器在浏览器历史中向前或向后移动。
...
给出了Selenium WebDriver最常用的一些浏览器导航命令。
...
在WebDriver中，这种方法使网络浏览器能够在现有的浏览器窗口中点击**前进**按钮。它既不接受任何东西，也不返回任何东西。
...
在WebDriver中，这种方法使网络浏览器能够在现有的浏览器窗口中点击**后退**按钮。它既不接受任何东西，也不返回任何东西。
...
让我们考虑一个示例测试脚本，它将涵盖WebDriver提供的大多数导航命令。
...
[https://www.testandquiz.com/selenium/testing.html](https://www.testandquiz.com/selenium/testing.html)(你也可以在你的Selenium测试实践中使用这个虚拟网页)
