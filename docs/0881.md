...
*   一台虚拟机用于配置了静态 IP 地址的Puppet主机。
*   一台虚拟机用于配置了静态 IP 地址的Puppet客户端。
...
首先，从两个虚拟机(即Puppet主机和Puppet代理)中删除防火墙规则。大多数 Linux 发行版中可用的默认防火墙是 iptables。在两台虚拟机上运行以下命令:
...
在Puppet客户端计算机上，键入Puppet代理的 IP 地址和主机名。我们还需要指定你的Puppet主人的 IP 地址和给它的主机名。
...
在以下命令的帮助下，用最新的包和存储库更新您的Puppet主代理和Puppet代理:
...
充当Puppet主机的Puppet服务器应该有准确的系统时间。您可能应该使用 NTP 服务来设置准确的系统时间。在您的 puppetServer 中使用以下命令来启用 NTP 服务器:
...
*   现在，我们将安装Puppet包，它将获取所有必要的先决条件和要求。

要安装Puppet主机，请在您的Puppet服务器中运行以下命令:
...
*   输出显示了Puppet主机及其运行服务的成功安装。要验证Puppet版本，请使用以下命令:
...
*   现在我们必须锁定Puppet版本的升级，因为这将妨碍升级Puppet时的配置。它不会在执行系统更新时更改 Puppet。我们可以通过创建一个具有如下配置的新文件来进行这些设置:
...
*   现在，我们将更改Puppet配置文件。在编辑器中打开 puppet server 的 puppet.conf 文件，并配置其参数，如下所示。
...
*   现在，按照我们对Puppet主服务器所做的相同步骤进行操作。我们必须锁定Puppet的版本，这样它就不会在操作系统更新过程中引起任何问题。
...
*   现在，我们将更改Puppet配置文件。在编辑器中打开 puppet 客户端的 puppet.conf 文件，并配置其参数，如下所示。
...
Puppet代理请求Puppet主人的证书，用于Puppet主人和Puppet代理的交流。因为在 puppet 客户端成功初始化后，它将检查 Puppet 主服务器，并在接受来自主 Puppet 服务器的任何管理指令之前发送证书请求。

一旦Puppet主人发送其证书，Puppet代理生成其自己的证书。然后它请求Puppet主人签署这个证书。在主人签署此证书后，Puppet主人和Puppet代理之间形成了安全链接。
...
上面的命令将创建一个证书颁发机构证书和一个Puppet主证书，其中包括适当的域名。

![Puppet Installation](img/75f232b2d24f5fab64f596042976fa28.png)

我们可以忽略这个警告信息。一旦您收到“注意:启动Puppet大师 5.4.0 版本”消息，请按 ctrl+c，因为我们现在无法启动Puppet大师。
...
*   现在，将证书签名请求从Puppet代理发送给Puppet主人。
...
*   签署请求的证书后，启动Puppet代理。要启动和启用Puppet代理，请运行以下命令:
