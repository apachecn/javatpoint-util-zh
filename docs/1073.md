...
要在WebDriver中使用断言，您需要下载 Testng jar 文件并将其添加到 eclipse 中。从下面给出的链接下载 Testng jar 文件:
...
到目前为止，我们已经了解了使用 Testng 框架的WebDriver中的硬断言。在硬断言中，如果断言失败，那么它将中止测试用例，否则它将继续执行。有时，即使断言失败，我们也希望执行整个脚本。这在硬断言中是不可能的。为了克服这个问题，我们需要在 testng 中使用一个软断言。